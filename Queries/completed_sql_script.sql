...
--Change the table name to verify that the data imported correctly
SELECT * FROM salaries;

--Change the name of the dept_employee table to correct a type during intialization of table
ALTER TABLE dept_empolyee
	RENAME TO dept_employee;
...
-- Joining departments and dept_manager tables using aliases to shorten the code
SELECT d.dept_name,
     dm.emp_no,
     dm.from_date,
     dm.to_date
...
-- Joining retirement_info and dept_employee tables using aliases to shorten the code
SELECT ri.emp_no,
    ri.first_name, 
	ri.last_name, 
    de.to_date
